...
@router.post("/create/{post_id}", status_code=status.HTTP_200_OK)
async def answer_create(post_id: int, _answer_create: answer_schema.AnswerCreate, db: AsyncSession = Depends(get_async_db), current_user: User = Depends(get_current_user)):

    post = await post_crud.get_post_id(db, post_id = post_id)
    board = await board_crud.get_board_id(db, board_id=post.board_id)
...
async def answer_delete(_answer_delete: answer_schema.AnswerDelete, db: AsyncSession = Depends(get_async_db), current_user: User = Depends(get_current_user)):
    ...
    await answer_crud.delete_answer(db=db, db_answer=db_post)

    return {"message": "삭제가 완료되었습니다"}


@router.get("/detail/{answer_id}", response_model=answer_schema.Answer)
async def answer_detail(answer_id: int, db: AsyncSession = Depends(get_async_db)):
    
    answer = await answer_crud.get_answer_id(db, answer_id=answer_id)
